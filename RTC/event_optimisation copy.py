...
for i, junction in enumerate(junctions):
    storage_curve = pd.read_csv(f"RTC/event_optimisation_input_data/storage_curve_{int(junction.split('_')[-1])}.dat", \
        delimiter=' ', skiprows=2, names=['depth', 'area'])
    max_storage = np.sum(storage_curve.loc[storage_curve.depth <= CSO_CREST_HEIGHT[i], 'depth'] \
...
for file_number in range(1, 5 + 1):
    output = swmm_api.read_out_file(fr"RTC\event_optimisation_output_data\Dean Town_pyswmm_{file_number}.out").to_frame()
    ...
    with pyswmm.Simulation(f"RTC\event_optimisation_input_data\Dean Town_pyswmm_{file_number}.inp") as sim:
        ...
        for step in sim:
            ...
            if (nodes['j_1'].depth > 0.2) or (nodes['j_20'].depth > 0.2):
                ...
                prob += (initial_filling[0] + time_step_size \
                ...
                prob += (initial_filling[1] + time_step_size \
                ...
                prob += (initial_filling[2] + time_step_size \
                ...
                prob += (initial_filling[3] + time_step_size \
                ...
                prob += (initial_filling[4] + time_step_size \
    ...
    break    
                ...
                # links["CSO_Pump_2"].target_setting = prob.variables()[6-1].varValue / CSO_PUMP_2_MAX
                # links["CSO_Pump_21"].target_setting = prob.variables()[7-1].varValue / CSO_PUMP_21_MAX
                # links["p10_1"].target_setting = prob.variables()[8-1].varValue / P_10_1_MAX
                # links["p_2_1"].target_setting = prob.variables()[9-1].varValue / P_2_1_MAX
                # links["p_20_2"].target_setting = prob.variables()[10-1].varValue / P_20_2_MAX
                # links["p_21_2"].target_setting = prob.variables()[11-1].varValue / P_21_2_MAX                  
                # links["WWTP_inlet"].target_setting = prob.variables()[12-1].varValue / WWTP_INLET_MAX
